...
/*   Updated: 2017/01/05 23:37:43 by tbouder          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_edit_arg_2(t_vm *env, t_champions *champ, int *nbr)
{
	...
	else if (IS_IND(nbr[1]) == 1)
		env->arg2 = env->map[ft_mod(pc + (env->arg2 % I), M)];
...
static int	ft_edit_arg_1(t_vm *env, t_champions *champ, int *nbr)
{
	...
	else if (IS_IND(nbr[0]) == 1)
		env->arg1 = env->map[ft_mod(pc + (env->arg1 % I), M)];
